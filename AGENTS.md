...
- **`hbt.analysis`** is the `hbt-analysis` executable, a Click group driving both libraries: `cli.py` gathers the commands; `commands/bench.py` and `commands/conformance.py` are the two commands, each translating its library's error into `CommandError`; `commands/__init__.py` holds what they share — the `--impl`/`--binary`/`--revision` options, deliberately on each command rather than the group (the `selection_options` docstring says why), and the exit-status handling; and `implementations.py` finds the implementations and their binaries.
