...
| `hbt-js` | TypeScript (npm) | github.com/henrytill/hbt-js |
| `hbt-ocaml` | OCaml (dune) | github.com/henrytill/hbt-ocaml |
| `hbt-rs` | Rust (cargo workspace) | github.com/henrytill/hbt-rs |

**`hbt-js` is the newest and least finished.** Its `bin/hbt` is currently a stub that prints `hbt` and exits 0, and every fixture is waived in its `conformance.waivers`, so `.#conformance` reports its column as `xfail`. The prose below still says "four" where it describes the mature implementations (hs, go, ocaml, rs); read it as five wherever `hbt-js` has caught up.
...
The same executable carries a `conformance` command (`hbt/analysis/commands/conformance.py`, `nix run .#conformance`), step 4 of `henrytill/hbt-data#14`: every implementation held to the corpus by `hbt.conformance`, the harness that lives in `hbt-data`, laid out one column per implementation. It imports the comparison rather than reimplementing it, and checks each implementation against the corpus *that implementation* pins — located by URL in its `.gitmodules`, because each implementation mounts it at its own path. The harness arrives as the `hbt-data` **flake input**, not a fifth submodule: `.gitmodules` is the list of implementations to `hbt.analysis.implementations` and both scripts, and the flake reads its `names` from `.gitmodules` too, rather than from the `hbt-` input prefix this input shares. Editing the harness in `/home/ht/src/hbt-data`? `--override-input hbt-data path:../hbt-data` puts the change in the matrix. Unlike the timings, the matrix *does* run in CI — the `info` workflow fails on it. A fixture an implementation does not satisfy yet is waived in that implementation's own `conformance.waivers` at its repository root, which the matrix finds by that convention and reads, the same way it finds the corpus by URL — so the matrix waives exactly what that implementation's own conformance run waives, and this repository holds no list of who is broken. Such a row reads `XFAIL` with the waiving issue as its reason; once the bug is fixed the fixture passes, the harness reports `XPASS`, and that *fails* the run, which is what takes the waiver back out. `--waivers NAME=FILE` replaces one implementation's file.

A third command, `fuzz` (`hbt/analysis/commands/fuzz.py`, `nix run .#fuzz`), holds the implementations to each other on generated documents rather than to the corpus: the place new fixtures come from, since conformance can only find what someone already pinned. Documents come from Hypothesis strategies in `hbt/analysis/generators.py`, listed simplest first because Hypothesis shrinks toward the first option. A disagreement is taken apart into *atoms* — a failure message some implementations give while others succeed, or a field (indices blanked) that splits them. The search runs in rounds with `report_multiple_bugs=False`, one atom to a round: the round's first new atom becomes its *target*, and only documents showing it fail. That target is what stops shrinking sliding into another disagreement — with one bug to report, Hypothesis shrinks toward any failure at all (a "slip", in its source), and deleting a date heading turns most finds into the undated-link one. An exception type per atom does not prevent that; it only separates bugs under multiple-bug reporting, which searches on a ten-second wall-clock timer and so would stop a seed reproducing a run. It runs with no example database and no deadline, and it is **not** a CI check: while the implementations disagree anywhere it exits non-zero, which today is always. hbt-js's stub fails every document, so leave it out with `--impl` until its CLI lands.
...
- Nothing pins `TZ`: all four are meant to be timezone-invariant, and the harness runs under the ambient zone so it can notice if one stops being so. `--tz` forces a zone when reproducing a failure.
...
- **Search all five trackers before filing or fixing:** `gh issue list -R henrytill/<repo> --state all` across `hbt-{hs,go,js,ocaml,rs}` and `hbt-data`. A question that looks new in one repo is usually already argued out — sometimes already decided and closed — in another.
...
- **`hbt.analysis`** is the `hbt-analysis` executable, a Click group driving both libraries: `cli.py` gathers the commands; `commands/bench.py`, `commands/conformance.py` and `commands/fuzz.py` are the three commands, the last with its Hypothesis strategies in `generators.py`; `commands/__init__.py` holds what they share — the `--impl`/`--binary`/`--revision` options, deliberately on each command rather than the group (the `selection_options` docstring says why), and the exit-status handling, which treats `CommandError`, `ImplementationError` and each library's own error alike as a refusal; and `implementations.py` finds the implementations, their binaries, their pinned corpora and their waivers files.
