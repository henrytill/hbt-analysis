...
- **`hbt.fuzz`** is the fuzzing library — `generators.py` (the Hypothesis strategies), `verdict.py` (running one implementation over a document and taking a disagreement apart into atoms), `search.py` (the one-atom-a-round search) and `report.py`. Like `hbt.bench` it knows nothing of `.gitmodules` or Click: its `Trial` is handed a mapping from implementation name to binary.
